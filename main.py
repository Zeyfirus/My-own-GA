...
class GA:
    def __init__(self,task_count,task_coef,time,dev_count,dev_coef):
        ...
        self.population_size = 600 # Размер популяции
        self.selected_size = 450
        self.fit = None
        self.population = self.create_population() # Начальная популяция
        self.n_steps = 30 # Поколения
        self.p = 0.1 # mutation
        self.elitism_size = 30
    ...
    def selection(self):
        ...
        while len(new_pop) < self.selected_size*self.task:
            ind1,ind2,ind3 = np.random.choice(self.population_size,3,replace=False)
            best_ind = min(self.fit[int(ind1)],self.fit[int(ind2)],self.fit[int(ind3)])
    ...
    def elitism(self):
        ...
        new_pop = np.array([], dtype=int)
        self.fitness(self.population)
        while len(new_pop)<self.elitism_size*self.task:
            elite=self.population[np.argmin(self.fitness(self.population))]
            new_pop=np.append(new_pop,elite)
        return new_pop.reshape(self.elitism_size,self.task)
    ...
    def crossover(self):
        ...
        for i in range((self.population_size-self.elitism_size) // 2):
            ...
            if prob <= p:
                ...
                new_gen = np.concatenate((new_gen, new_pop))
        return new_gen.reshape(self.population_size-self.elitism_size, self.task)


    def mutation(self):
        new = np.concatenate((self.crossover(),self.elitism()))
